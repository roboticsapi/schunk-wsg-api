...
public class SoftRobotReleasingMapper implements ActionMapper<SoftRobotRuntime, Releasing> {

	@Override
	public ActionMapperResult map(SoftRobotRuntime runtime, Releasing action, DeviceParameterBag parameters,
			ActionMappingContext ports) throws MappingException, RPIException {

		return new GoalActionMapperResult(action, new NetFragment("Releasing"),
				new ReleasingActionResult(action.getBaseJawWidth(), action.getSpeed()));
	}
