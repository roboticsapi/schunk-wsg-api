...
public class SoftRobotSetForceLimitMapper implements ActionMapper<SoftRobotRuntime, SetForceLimit> {

	@Override
	public ActionMapperResult map(SoftRobotRuntime runtime, SetForceLimit action, DeviceParameterBag parameters,
			ActionMappingContext ports) throws MappingException, RPIException {

		return new GoalActionMapperResult(action, new NetFragment("SetForceLimit"),
				new SetForceLimitActionResult(action.getForce()));
	}
