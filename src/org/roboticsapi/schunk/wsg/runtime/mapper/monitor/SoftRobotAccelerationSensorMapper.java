...
public class SoftRobotAccelerationSensorMapper
		implements SensorMapper<SoftRobotRuntime, Double, AbstractSoftRobotWSG50Driver.AccelerationSensor> {

	@Override
	public SensorMapperResult<Double> map(final SoftRobotRuntime runtime,
			final AbstractSoftRobotWSG50Driver.AccelerationSensor sensor, SensorMappingPorts ports,
			SensorMappingContext context) throws MappingException {
		final NetFragment ret = new NetFragment("SoftRobotVelocitySensor");
		final Monitor m = new Monitor(sensor.getDriver().getDeviceName());

		ret.add(m);

		DataflowOutPort port = ret.addOutPort(new DoubleDataflow(), true, m.getOutSpeed());
