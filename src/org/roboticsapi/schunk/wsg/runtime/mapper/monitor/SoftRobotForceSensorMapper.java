...
public class SoftRobotForceSensorMapper
		implements SensorMapper<SoftRobotRuntime, Double, AbstractSoftRobotWSG50Driver.ForceSensor> {

	@Override
	public SensorMapperResult<Double> map(final SoftRobotRuntime runtime,
			final AbstractSoftRobotWSG50Driver.ForceSensor sensor, SensorMappingPorts ports,
			SensorMappingContext context) throws MappingException {
		final NetFragment ret = new NetFragment("SoftRobotForceSensor");
		final Monitor m = new Monitor(sensor.getDriver().getDeviceName());

		ret.add(m);

		DataflowOutPort port = ret.addOutPort(new DoubleDataflow(), true, m.getOutForce());
