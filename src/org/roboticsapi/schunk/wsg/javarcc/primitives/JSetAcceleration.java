 ...
 */
public class JSetAcceleration extends JPrimitive {
	 ...
	 * capabilities.
	 */
	final JParameter<RPIdouble> propAcceleration = add("Acceleration", new JParameter<RPIdouble>(new RPIdouble("0")));
	...
	public void updateData() {
		...
		if (!inActive.isConnected() || inActive.get().get()) {
			if (init) {
				...
				// }
				// else {
				boolean result = device.setAcceleration(propAcceleration.get().get());
				if (!result) {
