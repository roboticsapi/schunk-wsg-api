 ...
 */
public class JSetForceLimit extends JPrimitive {
	 ...
	 * capabilities.
	 */
	final JParameter<RPIdouble> propForce = add("Force", new JParameter<RPIdouble>(new RPIdouble("0")));
	...
	public void updateData() {
		...
		if (!inActive.isConnected() || inActive.get().get()) {
			if (init) {
				...
				// }
				// else {
				boolean result = device.setForceLimit(propForce.get().get());
				if (!result) {
