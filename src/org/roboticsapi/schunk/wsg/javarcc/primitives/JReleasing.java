 ...
 */
public class JReleasing extends JPrimitive {
	...
	/** Opening width in [m]. */
	final JParameter<RPIdouble> propOpenWidth = add("OpenWidth", new JParameter<RPIdouble>(new RPIdouble("0")));
	...
	public void updateData() {
		...
		if (!inActive.isConnected() || inActive.get().get()) {
			if (init) {
				...
				// }
				// else {
				boolean result = device.release(propOpenWidth.get().get(), propSpeed.get().get());
				if (!result) {
