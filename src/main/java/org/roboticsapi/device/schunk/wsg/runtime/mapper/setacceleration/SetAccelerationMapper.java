...
public class SetAccelerationMapper implements ActionMapper<SetAcceleration> {
	@Override
	public ActionResult map(SetAcceleration action, DeviceParameterBag parameters, MapperRegistry registry,
			RealtimeBoolean cancel, RealtimeDouble override, RealtimeDouble time, Map<PlannedAction<?>, Plan> plans)
			throws MappingException, RpiException {
		return new SetAccelerationActionResult(action, RealtimeBoolean.TRUE, action.getAcceleration());
	}
}
