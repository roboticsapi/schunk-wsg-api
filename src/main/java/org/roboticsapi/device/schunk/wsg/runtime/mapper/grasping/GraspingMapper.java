...
public class GraspingMapper implements ActionMapper<Grasping> {

	@Override
	public ActionResult map(Grasping action, DeviceParameterBag parameters, MapperRegistry registry,
			RealtimeBoolean cancel, RealtimeDouble override, RealtimeDouble time, Map<PlannedAction<?>, Plan> plans)
			throws MappingException, RpiException {
		return new GraspingActionResult(action, RealtimeBoolean.TRUE, action.getBaseJawWidth(), action.getSpeed());
	}
