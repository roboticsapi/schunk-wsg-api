...
public class ReleasingMapper implements ActionMapper<Releasing> {

	@Override
	public ActionResult map(Releasing action, DeviceParameterBag parameters, MapperRegistry registry,
			RealtimeBoolean cancel, RealtimeDouble override, RealtimeDouble time, Map<PlannedAction<?>, Plan> plans)
			throws MappingException, RpiException {
		return new ReleasingActionResult(action, RealtimeBoolean.TRUE, action.getBaseJawWidth(), action.getSpeed());
	}
